...
def call_groq(system: str, user: str) -> dict:
    """Call Gemini API with model rotation to avoid rate limits/quotas."""
    ...
    # Pool of active Flash models to rotate through
    model_pool = [
    ...
    for attempt, model in enumerate(model_pool):
        ...
        try:
            ...
            if e.code in (429, 503):
                print(f"[MODEL ROTATOR] Model {model} failed with {e.code}. Rotating...")
                continue
            else:
                time.sleep(1)
        except Exception as e:
            print(f"[MODEL ROTATOR] Model {model} exception: {str(e)}. Rotating...")
            continue
            
    print("[MODEL ROTATOR] All models exhausted. Sleeping 5s and retrying...")
    time.sleep(5)
...
def compose_message(category: dict, merchant: dict, trigger: dict,
    ...
    chronic_count = cust_agg.get('high_risk_adult_count') or cust_agg.get('chronic_count') or cust_agg.get('high_risk_count')
    total_roster = cust_agg.get('total_unique_ytd') or cust_agg.get('total_roster') or 100
    if chronic_count:
    ...
    if conversation_history and len(conversation_history) > 0:
        ...
        last_from = last_turn.get("from") or last_turn.get("role")
        if last_from == "vera":
            last_body = (last_turn.get("body") or last_turn.get("content") or "").lower()
