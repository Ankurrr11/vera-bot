...
LLM_MODEL = "gemini-2.5-flash-lite"  # <-- Optional: specify model or leave empty for default
        ...
        for model in model_pool:
            ...
            try:
                ...
                if e.code in (429, 503):
                    print(f"\n[JUDGE ROTATOR] Model {model} failed with {e.code}. Rotating...")
                    continue
                else:
                    raise e
            except Exception as e:
                print(f"\n[JUDGE ROTATOR] Model {model} exception: {e}. Rotating...")
                continue

        print("\n[JUDGE ROTATOR] All models exhausted. Sleeping 5s and retrying...")
        time.sleep(5)
...
class GroqProvider(LLMProvider):
    ...
    def complete(self, prompt: str, system: str = None) -> str:
        ...
        req_data = json.dumps({"model": self.model, "messages": messages,
                                "temperature": 0.2, "max_tokens": 1500}).encode("utf-8")
        ...
        for attempt in range(5):
            try:
                ...
                if e.code == 429:
                    sleep_time = 25
                    print(f"\n[RATE LIMIT] Gemini 429. Sleeping {sleep_time}s and retrying...")
